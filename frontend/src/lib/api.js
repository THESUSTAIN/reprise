...
// ─── Agent Prospection (/api/prospection) ───────────────────────────────
// Les prospects détectés atterrissent dans la même table que le pipeline
// Croissance : ils sont donc visibles des deux côtés, sans synchronisation.
export const getIcp = () => api.get("/prospection/icp").then((r) => r.data);
export const saveIcp = (patch) => api.put("/prospection/icp", patch).then((r) => r.data);
export const getProspectionSources = () => api.get("/prospection/sources").then((r) => r.data);
// Un scan lit de vrais sites web : compter en dizaines de secondes, pas en
// millisecondes. Le timeout par défaut d'axios couperait la réponse en vol.
export const runProspectionScan = (body) => api.post("/prospection/scan", body, { timeout: 180000 }).then((r) => r.data);
export const getProspectionRuns = () => api.get("/prospection/runs").then((r) => r.data);
export const draftApproach = (body) => api.post("/prospection/brouillon", body, { timeout: 90000 }).then((r) => r.data);
export const getGdprNotice = () => api.get("/prospection/mention-information").then((r) => r.data);

// ─── Espace vendeur (/api/vendeur) ──────────────────────────────────────
// Marketplace multi-vendeurs : le vendeur dépose une fiche, Zayado la vérifie,
// puis elle est créée dans la boutique Shopify.
export const getVendeurEtat = () => api.get("/vendeur/etat").then((r) => r.data);
export const saveVendeurProfil = (patch) => api.put("/vendeur/profil", patch).then((r) => r.data);
export const getVendeurProduits = () => api.get("/vendeur/produits").then((r) => r.data);
export const createVendeurProduit = (p) => api.post("/vendeur/produits", p).then((r) => r.data);
export const updateVendeurProduit = (id, p) => api.put(`/vendeur/produits/${id}`, p).then((r) => r.data);
export const deleteVendeurProduit = (id) => api.delete(`/vendeur/produits/${id}`).then((r) => r.data);
export const submitVendeurProduit = (id) => api.post(`/vendeur/produits/${id}/soumettre`).then((r) => r.data);
export const checkVendeurProduit = (id) => api.get(`/vendeur/produits/${id}/verifier`).then((r) => r.data);
// Modération (équipe Zayado) — la publication appelle réellement Shopify.
export const getModerationQueue = () => api.get("/vendeur/moderation/attente").then((r) => r.data);
export const publishVendeurProduit = (id, vendeur_user_id) => api.post(`/vendeur/moderation/${id}/publier`, { vendeur_user_id }, { timeout: 60000 }).then((r) => r.data);
export const rejectVendeurProduit = (id, vendeur_user_id, motif) => api.post(`/vendeur/moderation/${id}/refuser`, { vendeur_user_id, motif }).then((r) => r.data);
