...
def user_to_response(user: User) -> UserResponse:
    ...
    # ── Trial calculation (free plan only) ──
    try:
        import os as _os
        from datetime import datetime as _dt, timezone as _tz, timedelta as _td
        trial_days = int(_os.environ.get("TRIAL_DURATION_DAYS", "14"))
        plan = _get("plan", "free") or "free"
        created = _get("created_at")
        if plan == "free" and created and trial_days > 0:
            created_aware = created.replace(tzinfo=_tz.utc) if created.tzinfo is None else created
            trial_end = created_aware + _td(days=trial_days)
            now = _dt.now(_tz.utc)
            seconds_left = (trial_end - now).total_seconds()
            days_left = max(0, int((seconds_left + 86399) // 86400))  # ceil to full days
            resp.trial_ends_at = trial_end
            resp.trial_days_left = days_left
            resp.trial_active = seconds_left > 0
        else:
            # Paid users or missing created_at → no trial badge
            resp.trial_ends_at = None
            resp.trial_days_left = None
            resp.trial_active = False
    except Exception:
        resp.trial_ends_at = None
        resp.trial_days_left = None
        resp.trial_active = False
