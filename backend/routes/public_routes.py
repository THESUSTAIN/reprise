...
async def get_public_admin_config():
    ...
    if config:
        ...
        safe_keys = ["demo_url", "social_instagram", "social_x", "social_tiktok", "social_linkedin", "social_youtube", "social_facebook", "whatsapp_url", "extension_url", "website_url", "footer_text", "footer_links", "footer_conditions_url", "footer_privacy_url", "footer_doc_url", "footer_copyright", "doc_url", "promo_banner", "logo_url", "extension_logo_url", "logo_white_url", "chat_logo_url", "header_help_url", "header_ecosystem"]
        result = {k: config.get(k, "") for k in safe_keys}
        # Default header_ecosystem (4 items) if never configured by admin
        if not result.get("header_ecosystem"):
            result["header_ecosystem"] = [
...
async def save_timer_session(data: dict, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        ), {"id": str(uuid.uuid4()), "uid": user.id, "dur": duration, "mode": mode, "src": source, "now": now_iso})
        await db.commit()
    except Exception:
        await db.rollback()
        # Table may not exist yet — create it silently (schéma portable SQLite/MySQL)
        try:
            await db.execute(sa_text(
...
async def get_timer_sessions(user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        return [{"id": r[0], "duration_minutes": r[1], "mode": r[2], "source": r[3],
                 "task_name": "", "created_at": (r[4].isoformat() if hasattr(r[4], "isoformat") else r[4])} for r in rows]
    except Exception:
        return []
