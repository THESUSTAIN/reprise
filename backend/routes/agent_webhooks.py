    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=60) as client:
            ...
            if resp.status_code == 200:
                ...
                logger.error(f"[agent_respond] Mammoth HTTP {resp.status_code}: {error_body}")
                if resp.status_code == 401:
                    reply = "Clé API invalide. Contactez le support Zayado."
                elif resp.status_code == 429:
                    reply = "Trop de requêtes. Attendez quelques secondes et réessayez."
                elif resp.status_code >= 500:
                    reply = "Le service IA est momentanément surchargé. Réessayez dans 30 secondes."
                else:
                    reply = f"Erreur API ({resp.status_code}). Réessayez dans quelques instants."
    except httpx.TimeoutException:
        logger.error("[agent_respond] Timeout après 60s sur Mammoth API")
        reply = "Je mets trop de temps à répondre. Réessayez dans un instant."
    except httpx.ConnectError as e:
        logger.error(f"[agent_respond] Connexion Mammoth impossible: {e}")
        reply = "Service IA momentanément indisponible. Réessayez dans 30 secondes."
    except Exception as e:
        logger.error(f"[agent_respond] Erreur inattendue: {type(e).__name__}: {e}")
        reply = "Erreur temporaire. Réessayez dans quelques instants."
