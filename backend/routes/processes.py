...
async def create_process(data: ProcessCreate, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    try:
        from sqlalchemy import text
        # Ensure table exists (schéma portable SQLite/MySQL — pas de NOW()/ON UPDATE)
        ...
        payload_json = json.dumps(process_data)
        # Upsert portable : UPDATE puis INSERT si aucune ligne touchée
        res = await db.execute(text(
            "UPDATE user_processes SET data = :data, updated_at = :now WHERE id = :id AND user_id = :uid"
        ), {"id": process_id, "uid": user.id, "data": payload_json, "now": now})
        if res.rowcount == 0:
            await db.execute(text(
                "INSERT INTO user_processes (id, user_id, data, created_at, updated_at) "
                "VALUES (:id, :uid, :data, :now, :now)"
            ), {"id": process_id, "uid": user.id, "data": payload_json, "now": now})
        await db.commit()
        return process_data
...
async def update_process(process_id: str, data: dict, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        ), {"id": process_id, "uid": user.id, "data": json.dumps(data), "now": datetime.now(timezone.utc).isoformat()})
        await db.commit()
        return {"ok": True}
    except Exception as e:
        await db.rollback()
