...
from utils import (
    ...
    load_admin_config, send_brevo_email,
)
...
async def upload_document_for_qa(file: UploadFile = File(...), user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    if not file.filename:
        raise HTTPException(status_code=400, detail="Fichier requis")
    ext = os.path.splitext(file.filename)[1].lower()
    if ext != ".pdf":
    ...
    try:
        from pypdf import PdfReader as _PdfReader
        import io as _io_doc
        _reader = _PdfReader(_io_doc.BytesIO(content))
        extracted_text = "\n".join(p.extract_text() or "" for p in _reader.pages)
    except Exception:
        raise HTTPException(status_code=500, detail="Erreur de lecture du document")
    # Texte extrait persisté en base (pas de fichier sur le disque du pod)
    settings = dict(user.settings or {})
    qa = dict(settings.get("qa_docs", {}))
    if len(qa) >= 20:
        for k in list(qa.keys())[:len(qa) - 19]:
            qa.pop(k, None)
    qa[doc_id] = {"text": extracted_text[:60000], "name": file.filename, "ts": datetime.now(timezone.utc).isoformat()}
    settings["qa_docs"] = qa
...
async def ask_document_question(doc_id: str, data: Dict[str, Any], user: User = Depends(get_current_user)):
    ...
    # Texte du document persisté en base (plus de fichier sur le disque du pod)
    _qa = (user.settings or {}).get("qa_docs", {})
    _doc = _qa.get(doc_id)
    if not _doc:
        raise HTTPException(status_code=404, detail="Document non trouve")
    extracted_text = _doc.get("text", "")
    mammoth_key = os.environ.get("MAMMOTH_API_KEY", "")
    if not mammoth_key:
...
async def get_smart_notifications(user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    # Notifications de démo investisseur : affichées UNIQUEMENT quand le mode
    # démo a été activé pour ce compte (flag posé par POST /api/demo/investor-fill).
    # Avant ce correctif elles étaient ajoutées pour TOUT le monde, sans
    # condition — un utilisateur réel avec un pipeline à 0 voyait "un lead à
    # fort potentiel" qui n'existait pas (signalé en QA comme trompeur).
    if (user.settings or {}).get("_demo_investor"):
