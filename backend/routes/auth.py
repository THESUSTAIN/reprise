...
async def register(user_data: UserCreateSchema, request: Request = None, db: AsyncSession = Depends(get_db)):
    ...
    <div style="font-family:'Segoe UI',Arial,sans-serif;max-width:600px;margin:0 auto;background:#ffffff;">
      ...
      <div style="background:#F8FAFC;padding:20px 24px;text-align:center;border-radius:0 0 12px 12px;border-top:1px solid #E2E8F0;">
        ...
        <p style="color:#CBD5E1;font-size:10px;margin:0;">ZAYADO — L'assistant IA des independants et TPE</p>
      </div>
    </div>"""
    asyncio.get_running_loop().run_in_executor(None, lambda: send_brevo_email(user.email, user.name, "Bienvenue sur ZAYADO — Votre assistant IA est pret !", welcome_html))
...
async def guest_login(db: AsyncSession = Depends(get_db)):
    """Mode aperçu (preview only) — crée ou réutilise un guest user."""
    if not _is_preview_env():
        raise HTTPException(status_code=403, detail="Mode invité indisponible en production.")

    guest_email = "guest@zayado.preview"
