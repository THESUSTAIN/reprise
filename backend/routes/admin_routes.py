...
async def get_ai_provider_setting(admin: User = Depends(get_admin_user)):
    """Retourne le fournisseur IA actif (auto | mammouth | emergent)."""
    import os as _os
    cfg = load_admin_config()
    provider = (cfg.get("ai_provider") or _os.environ.get("AI_PROVIDER", "auto")).lower()
    return {
        "provider": provider,
        "mammouth_configured": bool(_os.environ.get("MAMMOUTH_API_KEY") or _os.environ.get("MAMMOTH_API_KEY")),
        "emergent_configured": bool(_os.environ.get("EMERGENT_LLM_KEY")),
    }


@admin_router.put("/ai-provider")
async def set_ai_provider_setting(data: Dict[str, Any], admin: User = Depends(get_admin_user)):
    """Bascule le fournisseur IA en un clic (persisté dans admin_config.json)."""
    provider = (data.get("provider") or "").lower()
    if provider not in ("auto", "mammouth", "emergent"):
        raise HTTPException(status_code=400, detail="Fournisseur invalide (auto | mammouth | emergent)")
    cfg = load_admin_config()
...
async def admin_upload_asset(file: UploadFile = File(...), admin: User = Depends(get_admin_user)):
    """Upload logo/favicon — encodé en data URL (persiste dans la config, pas de disque pod)."""
    import base64 as _b64_asset
    content = await file.read()
    if len(content) > 5 * 1024 * 1024:
        raise HTTPException(status_code=400, detail="Fichier trop volumineux (max 5 Mo)")
    ext = (os.path.splitext(file.filename or "file")[1] or ".png").lstrip(".").lower()
    mime = {"png": "image/png", "jpg": "image/jpeg", "jpeg": "image/jpeg", "webp": "image/webp",
            "svg": "image/svg+xml", "gif": "image/gif", "ico": "image/x-icon"}.get(ext, "application/octet-stream")
    data_url = f"data:{mime};base64,{_b64_asset.b64encode(content).decode()}"
    return {"url": data_url, "filename": file.filename or f"asset.{ext}"}
...
SITE_URL_DEFAULTS = {
    ...
    # ── Header cockpit — bouton Aide (redirection WhatsApp support) ──
    "header_help_url": "",
    # ── Header cockpit — dropdown "Écosystème Zayado" (4 items éditables) ──
    "header_ecosystem": [
