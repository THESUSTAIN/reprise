...
async def _get_automations(db: AsyncSession, user_id: str) -> list:
    ...
    try:
        ...
        default_ids = {d["id"] for d in DEFAULT_AUTOMATIONS}
        merged = []
        for default in DEFAULT_AUTOMATIONS:
            stored_a = stored_map.get(default["id"], {})
            merged.append({**default, **stored_a})
        # Conserve les automatisations personnalisées (créées par l'utilisateur)
        for a in stored:
            if a.get("id") not in default_ids:
                merged.append(a)
        return merged
    except Exception:
        return [dict(a) for a in DEFAULT_AUTOMATIONS]
...
# Estimation du temps humain économisé par exécution (en minutes) — libellé « estimation ».
MINUTES_SAVED = {"a1": 20, "a2": 10, "a3": 15, "a4": 15, "a5": 45}


class CustomAutomationIn(BaseModel):
    description: str


class WellbeingConsentIn(BaseModel):
    enabled: bool
...
async def _get_automation_prefs(db: AsyncSession, user_id: str) -> dict:
    ...
    row = r.fetchone()
    if not row:
        return {"wellbeing_adaptive": False}
    try:
        return {**{"wellbeing_adaptive": False}, **json.loads(row[0])}
    except Exception:
        return {"wellbeing_adaptive": False}
...
@automations_router.get("/stats")
async def automations_stats(user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    """KPI « heures récupérées » (estimation ancrée sur les exécutions réelles) + consentement bien-être."""
    automations = await _get_automations(db, user.id)
    total_runs = sum(a.get("run_count", 0) for a in automations)
    minutes = sum(a.get("run_count", 0) * MINUTES_SAVED.get(a["id"], a.get("minutes_saved", 15)) for a in automations)
    active = sum(1 for a in automations if a.get("enabled"))
    prefs = await _get_automation_prefs(db, user.id)
    return {
        "active_count": active,
        ...
        "wellbeing_adaptive": prefs.get("wellbeing_adaptive", False),
    }


@automations_router.post("/custom")
async def create_custom_automation(body: CustomAutomationIn, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    """Crée un brouillon d'automatisation à partir d'une description en langage naturel (façon Emergent).
    Le brouillon est persisté, désactivé par défaut, avec statut « à configurer » (aucune exécution fictive)."""
    ...
    await _save_automations(db, user.id, automations)
    return {"status": "ok", "automation": new_auto}


@automations_router.delete("/custom/{auto_id}")
async def delete_custom_automation(auto_id: str, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    automations = await _get_automations(db, user.id)
    kept = [a for a in automations if not (a["id"] == auto_id and a.get("custom"))]
    await _save_automations(db, user.id, kept)
    return {"deleted": len(kept) != len(automations)}


@automations_router.put("/wellbeing-consent")
async def set_wellbeing_consent(body: WellbeingConsentIn, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    """Consentement explicite (RGPD Art. 9) : autoriser l'IA à ADAPTER (suggérer, jamais exécuter)
    les automatisations selon l'énergie de l'utilisateur. Désactivé par défaut, révocable."""
    prefs = await _get_automation_prefs(db, user.id)
    prefs["wellbeing_adaptive"] = body.enabled
    await _save_automation_prefs(db, user.id, prefs)
    return {"status": "ok", "wellbeing_adaptive": body.enabled}
