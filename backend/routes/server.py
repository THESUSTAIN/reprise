...
# Missing APIs (collaborateur/notify, tasks, mission, vision, etc.)
from routes.missing_apis import missing_router
app.include_router(missing_router, prefix="/api")

# Agent webhooks (public — no /api prefix needed, but included for ingress routing)
                ...
                for col, defn in cols:
                    try:
                        ...
                        if not _re_migrate.match(r'^[a-z_]+$', table) or not _re_migrate.match(r'^[a-z_]+$', col):
                            ...
                            continue
                        # Skip if column already exists (avoids noisy errors on every boot)
                        try:
                            insp = await db.execute(text(f"SELECT 1 FROM {table} LIMIT 1"))
                            insp.close()
                        except Exception:
                            pass
                        try:
                            ...
                            await db.commit()
                            logger.info(f"[AUTOMIGRATE] +{table}.{col}")
                        except Exception as col_err:
                            msg = str(col_err).lower()
                            await db.rollback()
                            # Silently skip 'duplicate column' / 'already exists' errors
                            if 'duplicate' not in msg and 'already exists' not in msg and 'already contains' not in msg:
                                logger.warning(f"[AUTOMIGRATE] {table}.{col}: {col_err}")
                    except Exception as outer_err:
                        logger.exception(f"[AUTOMIGRATE] Outer failure on {table}.{col}: {outer_err}")
                        await db.rollback()
