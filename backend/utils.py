...
def send_brevo_email(to_email: str, to_name: str = "", subject: str = "", html_content: str = "", brand: str = "zayado", html: str = None, attachments: list = None):
    ...
    Args:
        ...
        attachments: liste optionnelle [{"content": <base64 str>, "name": "facture.pdf"}]
    """
    ...
    try:
        ...
        send_smtp_email = sib_api_v3_sdk.SendSmtpEmail(**kwargs)
...
def invoice_number(tx) -> str:
    """Numéro de facture lisible et stable basé sur la transaction."""
    created = getattr(tx, "created_at", None) or datetime.now(timezone.utc)
    try:
        ym = created.strftime("%Y%m")
    except Exception:
        ym = datetime.now(timezone.utc).strftime("%Y%m")
    short = (getattr(tx, "id", "") or "")[:8].upper()
    return f"ZAY-{ym}-{short}"


def generate_invoice_pdf(tx, user, plan_name: str = "") -> bytes:
    """Génère une facture PDF (bytes) pour une transaction payée — brand Zayado navy/or.

    Pure Python (reportlab), aucune dépendance externe / clé requise.
    """
    ...
    ttc = amount
    ht = round(ttc / 1.20, 2)      # TVA 20% incluse
    tva = round(ttc - ht, 2)

    tx_type = (getattr(tx, "type", "") or "").lower()
    if tx_type == "subscription":
        label = f"Abonnement {plan_name or (getattr(tx, 'package_id', '') or '').upper()}"
    elif tx_type == "credits":
        label = f"Pack de crédits ({getattr(tx, 'credits', 0)} crédits)"
    else:
        label = getattr(tx, "package_id", None) or "Prestation Zayado"
    ...
    c.drawString(20 * mm, H - 22 * mm, "ZAYADO")
    c.setFillColor(colors.white)
    c.setFont("Helvetica", 9)
    c.drawString(20 * mm, H - 29 * mm, "MyExtension AI by Zayado")
    c.drawString(20 * mm, H - 34 * mm, "zayado.net · noreply@zayado.net")
    c.setFont("Helvetica-Bold", 16)
    c.drawRightString(W - 20 * mm, H - 22 * mm, "FACTURE")
    c.setFont("Helvetica", 9)
    c.drawRightString(W - 20 * mm, H - 29 * mm, f"N° {inv_no}")
    c.drawRightString(W - 20 * mm, H - 34 * mm, f"Date : {date_str}")
    ...
    c.drawString(20 * mm, y - 6 * mm, getattr(user, "name", None) or "Client")
    c.setFillColor(colors.black)
    c.setFont("Helvetica", 10)
    c.drawString(20 * mm, y - 12 * mm, getattr(user, "email", "") or "")
    ...
    c.drawRightString(W - 24 * mm, ty + 2.7 * mm, "MONTANT")

    c.setFillColor(colors.black)
    c.setFont("Helvetica", 10)
    ry = ty - 10 * mm
    c.drawString(24 * mm, ry, label)
    c.drawRightString(W - 24 * mm, ry, f"{ht:.2f} €")

    # ── Totaux ──
    c.setStrokeColor(colors.HexColor("#E5E7EB"))
    c.line(20 * mm, ry - 6 * mm, W - 20 * mm, ry - 6 * mm)
    tot_y = ry - 14 * mm
    c.setFont("Helvetica", 10)
    c.setFillColor(GREY)
    c.drawRightString(W - 55 * mm, tot_y, "Total HT")
    c.drawRightString(W - 55 * mm, tot_y - 6 * mm, "TVA (20%)")
    c.setFillColor(colors.black)
    c.drawRightString(W - 24 * mm, tot_y, f"{ht:.2f} €")
    c.drawRightString(W - 24 * mm, tot_y - 6 * mm, f"{tva:.2f} €")
    c.setFillColor(NAVY)
    c.rect(W - 90 * mm, tot_y - 20 * mm, 70 * mm, 9 * mm, fill=1, stroke=0)
    c.setFillColor(colors.white)
    c.setFont("Helvetica-Bold", 11)
    c.drawString(W - 87 * mm, tot_y - 17.3 * mm, "TOTAL TTC")
    c.setFillColor(GOLD)
    c.drawRightString(W - 24 * mm, tot_y - 17.3 * mm, f"{ttc:.2f} €")

    # ── Statut paiement ──
    c.setFillColor(colors.HexColor("#2E7D5B"))
    c.setFont("Helvetica-Bold", 10)
    c.drawString(20 * mm, tot_y - 17.3 * mm, "PAYÉE")
    if getattr(tx, "mollie_payment_id", None):
        c.setFillColor(GREY)
        c.setFont("Helvetica", 8)
        c.drawString(20 * mm, tot_y - 23 * mm, f"Réf. paiement : {tx.mollie_payment_id}")

    # ── Footer ──
    c.setFillColor(GREY)
    c.setFont("Helvetica", 8)
    c.drawCentredString(W / 2, 18 * mm, "Merci de votre confiance — Zayado, entreprendre avec sens, clarté et bien-être.")
    c.drawCentredString(W / 2, 13 * mm, "TVA non applicable / incluse selon régime. Document généré automatiquement, tenant lieu de facture.")
